...
const CONDITION_LINE = preload("res://addons/HTNDomainManager/PluginSystem/ConditionEditor/condition_line.tscn")

@onready var method_nick_name: LineEdit = %MethodNickName
@onready var condition_parent: VBoxContainer = %ConditionParent
...
func open(node: HTNMethodNode, data: Dictionary) -> void:
	show()
	_method_node = node
	method_nick_name.text = _method_node._nick_name
...
func _create_and_load_condition(data: Dictionary={}) -> void:
	...
	condition_parent.add_child(condition_line)
